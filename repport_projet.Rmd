...
<-- Discuter des problèmes et solutions --!>
...
vec_na_T0<-apply(X = data_physio[,5:21],MARGIN = 1, FUN = function(x){sum(is.na(x))})
vec_na_T1<-apply(X = data_physio[,22:38],MARGIN = 1, FUN = function(x){sum(is.na(x))})
vec_na_T0_T1<-apply(X = data_physio[,5:38],MARGIN = 1, FUN = function(x){sum(is.na(x))})
```
...
data_physio<-data_physio[vec_na_T0_T1<=17,]
...
```{r, echo=FALSE}
T0_indi_sp<-apply(apply(data_physio[vec_na_T0==0,5:21],2, cherche.ext),1,sum)==17
T1_indi_sp<-apply(apply(data_physio[vec_na_T1==0,22:38],2, cherche.ext),1,sum)==17
```
...
imput_T0<-apply(data_physio[vec_na_T0==0,5:21],2, FUN=function(x){
...
apply(imput_T0,2,function(x){sum(is.na(x))/length(x)*100})
...
```{r, echo=FALSE}
init_T0 = mice(imput_T0, maxit=0) 
meth = init_T0$method
predM = init_T0$predictorMatrix

imputed_T0 <- mice(imput_T0, method=meth, predictorMatrix=predM, m=5)
imput_T0_mean <- complete(imputed_T0)
```
...
imput_T1<-apply(data_physio[vec_na_T1==0,22:38],2, FUN=function(x){
  need_replace<-!(cherche.ext(x))
  xbis<-x
  xbis[need_replace]<-NA
  return(xbis)})
...
```{r, echo=FALSE}
init_T1 = mice(imput_T1, maxit=0) 
meth = init_T1$method
predM = init_T1$predictorMatrix

imputed_T1 <- mice(imput_T1, method=meth, predictorMatrix=predM, m=5)
imput_T1_mean <- complete(imputed_T1)
```
