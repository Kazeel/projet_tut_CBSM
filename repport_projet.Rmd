...
#### RMSE (Root Mean Squared Error), MAE (Mean Absolute Error) et MAPE (Mean Absolute Pourcentage Error)

Afin de comparer nos méthodes d'imputation, nous allons utiliser le RMSE ou l'erreur quadratique, il s'agit simplement de la racine carré de l'espérance des carrés de la différence entre estimation et valeure réelle.

Une autre mesure est le Mean absolute error qui est simplement la valeur absolue de la différence entre prediction et réalité. On peut aussi voir cette mesure selon le pourcentage d'erreur et on parlera alors de MAPE.

#### Predictive mean matching

Parmis les méthodes d'imputation, si la plus classique est simplement d'utiliser la moyenne comme nouvelle valeur, il existe d'autres méthodes plus poussée. L'une d'entre elle est la predictive mean matching. Le principe est simplement de chercher l'individus complet le plus proche de l'individus à qui il manque une valeur puis de remplacer la valeur manquante par celle de l'individus proche. C'est une méthode assez populaire et qui se présente comme assez robuste.
...
Cette méthode sera utilisé dans le cadre de la recherche d'un modèle de prédiction.
...
La validation croisée est une méthode d'estimation de la fiabilité basé sur une technique d'échantillonage. Le principe est de découper nos données d'entrainements en plusieurs partition. A chaque itération, on utilisera une partition comme test et les autres comme données pour entrainer le modèle.
...
Notre but premier est de remplacer les valeurs extrêmes chez nos individus afin de conserver un maximum d'individus pour la suite de l'étude.
...
On observe que de manière générale, les variables n'ont pas un taux trop important de valeurs abérantes. Nous allons donc imputer l'ensemble des variables d'un seul coup.

Nous avons plusieurs choix possible de méthode pour l'imputation. Pour les comparer, nous allons donc volontairement retirer des valeurs parmis les données connus.Considérons notre tableau de données sans les NA, puis créons artificiellement et aléatoirement 5% de valeurs manquantes.
...
Ce package va nous permettre d'utiliser 5 méthodes d’imputations. Il s’agit de la Predictive mean matching PMM, la moyenne MEAN, random forest RF, Random sample for observed values SAMPLE et une dernière le Bayesian Linear Regression NORM.

Le critère de comparaison est axé sur la minimisation des erreurs de prédiction le RMSE entre les valeurs prédites par les imputations et les valeurs réelles. De plus en observant les statistiques descriptives avant et après les imputations, nous confirmerons de pas altérer de façon drastique nos données.

Pour le choix du meilleur modèle d’imputation, choisir celle qui minimise au mieux sur l’ensemble des 10 variables et aussi en gardant un oeil sur la variable RMSSD, qui sera importante pour la suite (définition de l'amélioration de l'état physique d'un individus).

Après quelques essaies, nous avons retenu 3 types d’imputations efficacent sur nos données. Le PMM et le Bayesian linear régression et la moyenne.
...
```{r,echo=FALSE}
#summary(imputed_Data_mean) ILLISIBLE DANS LE RAPPORT
...
#summary(imputed_Data_cart) ILLISIBLE DANS UN RAPPORT
...
#summary(imputed_Data_norm) NO SUMMARY, ILLISIBLE DANS UN RAPPORT
...
levels(merge_data$"progress")<-c("yes","no")
...
roc_glm_loo <- ggplot(m_glm_loo$pred, aes(m=yes, d=factor(obs, levels = c("yes", "no")))) + 
  ...
  annotate("text", x=0.75, y=0.25, label=paste("AUC =", round((calc_auc(roc_glm_loo))$AUC, 4)))
...
roc_glm_boot <- ggplot(m_glm_boot$pred, aes(m=yes, d=factor(obs, levels = c("yes", "no")))) + 
  ...
  annotate("text", x=0.75, y=0.25, label=paste("AUC =", round((calc_auc(roc_glm_boot))$AUC, 4)))

grid.arrange(roc_glm_loo,roc_glm_boot, nrow=1, ncol=2)
```
...
roc_svm_loo <- ggplot(m_svm_loo$pred, aes(m=yes, d=factor(obs, levels = c("yes", "no")))) + 
  ...
  annotate("text", x=0.75, y=0.25, label=paste("AUC =", round((calc_auc(roc_svm_loo))$AUC, 4)))
...
roc_svm_boot <- ggplot(m_svm_boot$pred, aes(m=yes, d=factor(obs, levels = c("yes", "no")))) + 
  ...
  annotate("text", x=0.75, y=0.25, label=paste("AUC =", round((calc_auc(roc_svm_boot))$AUC, 4)))


grid.arrange(roc_svm_loo,roc_svm_boot, nrow=1, ncol=2)
```
...
roc_rf_loo <- ggplot(m_rf_loo$pred, aes(m=yes, d=factor(obs, levels = c("yes", "no")))) + 
  ...
  annotate("text", x=0.75, y=0.25, label=paste("AUC =", round((calc_auc(roc_rf_loo))$AUC, 4)))
...
roc_rf_boot <- ggplot(m_rf_boot$pred, aes(m=yes, d=factor(obs, levels = c("yes", "no")))) + 
  ...
  annotate("text", x=0.75, y=0.25, label=paste("AUC =", round((calc_auc(roc_rf_boot))$AUC, 4)))

grid.arrange(roc_rf_loo,roc_rf_boot, nrow=1, ncol=2)
```
...
Concernant les modèles de prédictions, nous pouvons voir qu'avec nos données, il semble difficile d'établir un modèle convenable. Même si nous utilisons différentes méthodes pour la recherche de predicteur efficace, nous avons au final une précision faible dans tout les cas. Il semble alors pour le moment difficile de prédire l'amélioration de l'état de santé d'un individus via le programme en se basant seulement sur des informations physilogique avant expérience.

Enfin, pour conclure, il est a noté qu'il est possible que la méthode du RandomForest puisse renvoyer de meilleur prédiction. En effet, il suffirait d'inverser le modèle et nous aurions alors un modèle de prédiction avec une précision plus grande. Ainsi, on peut imaginer qu'avec beaucoup plus de données et surement d'itération, il est possible de voir apparaître un modèle acceptable. 