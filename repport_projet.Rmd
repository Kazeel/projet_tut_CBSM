...
L'imputation est le processus de remplacement des données manquantes par des valeurs substituées. Il existe plusieurs techniques d'imputations, le but est donc de les Comparer pour voir laquelle donne de meilleures résultats.
Nous allons utiliser le package MICE (Mutivariate Imputation via Chained Equations) qui permet d'utiliser plusieurs techniques d'imputations pour traiter les données manquantes.
Nous avons tout d'abord récupéré la partie du jeu de données sans les NA afin de recréer des valeurs manquantes artificiellement et de comparer plus tard les résultats des différentes méthodes d’imputations que nous allons effectuer sur le jeu de données. 

5 techniques d’imputations ont été expérimentées:
 *  Predictive mean matching(pmm):
 *  la moyenne(mean): utilise la moyenne obtenue sans les valeurs manquantes et impute la même valeur à l’ensemble des valeurs manquantes
 *  random forest(rf):
 *  Random sample for observed values(sample):
...
Le RMSE est la racine carrée du MSE qui mesure essentiellement l'erreur quadratique moyenne de nos prévisions. Pour chaque point, il calcule la différence carrée entre les prévisions et les vraies valeurs, puis fait la moyenne de ces valeurs.
La meilleure technique est donc celle qui a les plus petits RMSE par variable.
...
```
On observe que de manière générale, les variables n'ont pas un taux trop important de valeurs abérantes. 
...
Comparons tout d'abord plusieurs techniques d'imputations pour voir laquelle est la meilleure. 
...
- m= le nombre imputation par NA (ici 1)
...
Maintenant que nous avons imputé nos variables, nous pouvons chercher à créer un modèle pour prédire si une personne a vu sa condition physique s'améliorée.
...
Avec le SVM, nous obtenons une très bonne accuracy (jusqu'à 90% selon l'hyper paramètre). C'est un excellent résultat dans notre cas.
...
On observe ici que l'optimisation du paramètre C avec le bootstrap renvoie plus ou moins le même résultat pour le choix de l'hyper paramètre (le paramètre peut varier mais reste dans le même ordre de grandeur). Néanmoins, la précision est en général plus faible que avec le Leave One Out. On peut supposer que la plus grande diversité d'échantillon dans nos tests dû à la méthode en est la cause. Mais nous conservons des résultats concluants (bonne précision)
...
Même si l'aspect aléatoire de cette méthode rend encore difficile l'évaluation, l'utilisation du bootstrap permet néanmoins d'observer plus facilement une tendance : la diminution de la précision à mesure que l'on rajoute des variables.
